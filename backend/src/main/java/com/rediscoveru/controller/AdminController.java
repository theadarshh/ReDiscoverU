...
public class AdminController {
    ...
    public ResponseEntity<?> updatePrice(@RequestBody Map<String, Object> body) {
        try {
            ...
            Object lpVal = body.get("launchpadPrice");
            if (lpVal != null) s.setLaunchpadPrice(new BigDecimal(lpVal.toString()));
            Object lfVal = body.get("lifetimePrice");
            if (lfVal != null) s.setLifetimePrice(new BigDecimal(lfVal.toString()));
            s.setId(1L);
            settingsRepo.save(s);
