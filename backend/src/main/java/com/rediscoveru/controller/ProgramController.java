...
import com.rediscoveru.repository.AnnouncementRepository;
...
public class ProgramController {
    ...
    public ResponseEntity<?> announcements(@AuthenticationPrincipal UserDetails ud) {
        if (ud == null) return ResponseEntity.status(401).body(Map.of("error", "Login required"));
        try { return ResponseEntity.ok(announcementRepo.findByActiveTrueAndExpiresAtAfterOrderByDisplayOrderAscCreatedAtDesc(java.time.LocalDateTime.now())); }
