...
public class AdminService {
     ...
     * Guards: cannot change own role; cannot demote the primary admin.
     */
    public void changeRole(Long userId, User.Role newRole,
                           String requesterEmail, String primaryAdminEmail) {
        User u = userRepo.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));
        if (u.getEmail().equalsIgnoreCase(requesterEmail))
            throw new RuntimeException("You cannot change your own role");
        if (u.getEmail().equalsIgnoreCase(primaryAdminEmail) && newRole == User.Role.ROLE_USER)
            throw new RuntimeException("The primary administrator cannot be demoted");
        u.setRole(newRole);
        userRepo.save(u);
    }
