...
public class MentorService {
    ...
    /**
     * Upload mentor photo.
     * Uses Cloudflare R2 if configured, else local disk.
     */
    public Mentor uploadImage(Long id, MultipartFile file) throws Exception {
        Mentor m = mentorRepo.findById(id)
            .orElseThrow(() -> new RuntimeException("Mentor not found"));

        String imageUrl;
        if (cloudStorageService.isEnabled()) {
            imageUrl = cloudStorageService.upload(file, "mentors");
        } else {
            ...
            imageUrl = "/uploads/mentors/" + filename;
        }
    ...
    public void seedDefault() {
        if (mentorRepo.count() == 0) {
            ...
            m.setBio("Jayashankar is a Lifestyle and Mindset Coach dedicated to helping students " +
                "and working professionals build structured, meaningful lives. His approach blends " +
                "practical habit design, emotional intelligence, and personal accountability — " +
