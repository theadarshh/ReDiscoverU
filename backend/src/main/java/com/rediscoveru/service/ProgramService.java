...
public class ProgramService {
    ...
    public Program create(Program p) {
        ...
        // Notify all paid users of the new program
        try { emailNotificationService.notifyPaidUsersNewProgram(saved); } catch (Exception ignored) {}
        return saved;
    }
    ...
    public ProgramContent addContent(Long programId, ProgramContent content) {
        ...
        ProgramContent saved = contentRepo.save(content);
        // Notify paid users about new content
        try {
            String programTitle = saved.getProgram() != null ? saved.getProgram().getTitle() : "ReDiscoverU";
            String contentTitle = saved.getTitle() != null ? saved.getTitle() : "New Content";
            emailNotificationService.notifyPaidUsersNewContent(programTitle, contentTitle);
    ...
    public LiveSession addSession(Long programId, LiveSession session) {
        ...
        // Notify all paid users of the new session
        try { emailNotificationService.notifyPaidUsersNewSession(saved); } catch (Exception ignored) {}
        return saved;
    }
    ...
    public String buildScheduleText(LiveSession s) {
        ...
        switch (s.getRecurrenceType()) {
            ...
            case WEEKENDS            -> sb.append("Sat & Sun");
